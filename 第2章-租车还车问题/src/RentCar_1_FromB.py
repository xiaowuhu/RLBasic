...
if __name__ == "__main__":
    ...
    print("天数 =",3)
    for return_to in RentalStore:
        num_from, num_to = Statistic(data_array, rent_from, return_to, t=3)
        print(str.format("从 {0} 店出租 {2} 次，还到 {1} 店 {3} 次", rent_from.name, return_to.name, num_from, num_to))

    print("天数 =",40)
    for return_to in RentalStore:
        num_from, num_to = Statistic(data_array, rent_from, return_to, t=40)
        print(str.format("从 {0} 店出租 {2} 次，还到 {1} 店 {3} 次", rent_from.name, return_to.name, num_from, num_to))


    print("天数 =",50)
    for return_to in RentalStore:
        num_from, num_to = Statistic(data_array, rent_from, return_to, t=50)
        print(str.format("从 {0} 店出租 {2} 次，还到 {1} 店 {3} 次", rent_from.name, return_to.name, num_from, num_to))
